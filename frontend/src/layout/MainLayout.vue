<template>
  <div class="ghost-faces-layer" aria-hidden="true">
    <span class="ghost-face"></span><span class="ghost-face"></span><span class="ghost-face"></span>
    <span class="ghost-face"></span><span class="ghost-face"></span><span class="ghost-face"></span>
  </div>
  <div class="blood-drops" aria-hidden="true">
    <span class="blood-drop"></span><span class="blood-drop"></span><span class="blood-drop"></span>
    <span class="blood-drop"></span><span class="blood-drop"></span>
  </div>
  ...
  <el-container style="height: 100vh; position: relative; z-index: 1;">
    <el-aside width="220px" style="background: linear-gradient(180deg, #0d0a08 0%, #110d08 100%); border-right: 1px solid rgba(122,0,0,0.2);">
      ...
      <el-menu :default-active="route.path" router background-color="#0d0a08" text-color="#8a7a68" active-text-color="#c9a84c">
        ...
        <el-menu-item index="/soul"><el-icon><User /></el-icon><span>魂魄管理</span></el-menu-item>
        <el-menu-item index="/judgment" v-if="userStore.roleCode === ROLE.JUDGE"><el-icon><Document /></el-icon><span>审判管理</span></el-menu-item>
        <el-menu-item index="/review" v-if="userStore.roleCode === ROLE.KING"><el-icon><Checked /></el-icon><span>复核管理</span></el-menu-item>
        <el-menu-item index="/rebirth" v-if="userStore.roleCode === ROLE.KING || userStore.roleCode === ROLE.ADMIN"><el-icon><Refresh /></el-icon><span>轮回管理</span></el-menu-item>
        <el-menu-item index="/hell"><el-icon><Warning /></el-icon><span>十八层地狱</span></el-menu-item>
        <el-menu-item index="/demon"><el-icon><MagicStick /></el-icon><span>百鬼录</span></el-menu-item>
        <el-sub-menu index="system" v-if="userStore.roleCode === ROLE.ADMIN">
...
async function startAmbientAudio() {
  ...
  } catch (e) { console.log('播放被浏览器拦截，请点击阴乐按钮') }
}
...
<style scoped>
.ghost-faces-layer { position: fixed; inset: 0; pointer-events: none; z-index: 0; overflow: hidden; }
.ghost-face {
  position: absolute; border-radius: 50% 50% 10% 10%;
  background: radial-gradient(ellipse at 50% 30%, rgba(255,255,255,0.04) 0%, transparent 70%);
  filter: blur(12px); animation: wispFloat 14s ease-in-out infinite;
}
...
.blood-drop {
  ...
  border-radius: 0 0 50% 50%; animation: bloodDrip linear infinite; opacity: 0.5;
}
...
.logo { height: 60px; display: flex; align-items: center; justify-content: center; gap: 10px; color: var(--gold, #c9a84c); font-family: var(--font-display); font-size: 22px; font-weight: 400; border-bottom: 1px solid rgba(122,0,0,0.3); background: linear-gradient(180deg, rgba(122,0,0,0.08), transparent); animation: candleFlicker 4s ease-in-out infinite; }
