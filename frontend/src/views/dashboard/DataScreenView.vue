    ...
    <div class="particles" aria-hidden="true">
      <span v-for="i in 30" :key="i" class="particle"
        :style="{
          ...
          animationDuration: (8 + rand(i*5)) + 's',
          fontSize: (14 + rand(i*11)) + 'px',
          opacity: 0.03 + rand(i*7) * 0.01
        }"
        ...
        <div class="panel panel-feed">
          ...
          <div class="feed-wrap">
            <div class="feed-scroll" ref="feedScroll">
              <div v-for="(item, i) in activityFeed" :key="i" class="feed-row"
      ...
      <div class="center-col">
        <div class="stats-grid">
          <div class="stat-item" v-for="s in statsCards" :key="s.label"
            ...
            <div class="stat-body">
              <span class="stat-num" :style="{ color: s.color }" ref="statNums">{{ animated[s.key] ?? s.value }}</span>
...
function updateClock() {
  const now = new Date()
  dateStr.value = now.getFullYear() + '年' +
    String(now.getMonth() + 1).padStart(2, '0') + '月' +
    String(now.getDate()).padStart(2, '0') + '日'
  timeStr.value = String(now.getHours()).padStart(2, '0') + ':' +
...
function updatePie(data) {
  ...
  pieChartInst.setOption({
    ...
    series: [{
      ...
      roseType: 'area',
      data: data.map((d, i) => ({
        name: names[d.path - 1] || '未知',
        value: d.count,
        itemStyle: { color: colors[d.path - 1] || '#666', borderRadius: 6, borderColor: '#0a0a0f', borderWidth: 3 }
      })),
...
.skull {
  font-size: 38px;
  animation: skullStare 2s ease-in-out infinite;
